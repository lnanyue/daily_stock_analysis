...
| `OPENBB_FETCHER_PROVIDER` | OpenBB 行情 provider，例如 `yfinance`、`fmp`、`intrinio`（取决于本地 OpenBB 扩展与凭证），默认 `yfinance` | 可选 |
...
| `NEWS_MAX_AGE_DAYS` | 新闻最大时效（天），搜索时限制结果在近期内 | 默认 `7` |
| `BIAS_THRESHOLD` | 乖离率阈值（%），超过提示不追高；强势趋势股自动放宽到 1.5 倍 | 默认 `5.0` |

> 个股分析的综合情报会同时拉取个股/行业新闻与宏观新闻。宏观新闻主要覆盖美联储/央行利率、通胀、美债收益率、美元/人民币或港元汇率、政策与流动性变化，用于判断估值、资金面和市场风险偏好是否支持当前交易结论。
