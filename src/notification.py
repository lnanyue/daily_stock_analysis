...
from src.notification.renderer import ReportRenderer
from src.notification.utils import get_source_display_name
...
class NotificationService:
    ...
    def _detect_all_channels(self) -> List[NotificationChannel]:
        ...
        if config.discord_bot_token and config.discord_main_channel_id: channels.append(NotificationChannel.DISCORD)
    ...
    async def _send_channel_with_retry(self, channel, content, image_bytes, email_stock_codes, email_send_to_all) -> bool:
        for attempt in range(self._notification_max_retries + 1):
            try:
                # Basic routing logic here... (simplified for brevity)
                if channel == NotificationChannel.WECHAT: return await self.send_to_wechat(content)
                if channel == NotificationChannel.FEISHU: return await self.send_to_feishu(content)
                if channel == NotificationChannel.EMAIL: return await self.send_to_email(content)
                if channel == NotificationChannel.TELEGRAM: return await self.send_to_telegram(content)
                # ... other channels
                return await self._send_single_channel(channel, content)
            except Exception:
                if attempt == self._notification_max_retries: return False
