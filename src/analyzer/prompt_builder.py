...
4. ❓ 量能是否配合（缩量回调/放量突破）？
5. ❓ 筹码结构是否健康？
6. ❓ 宏观利率、流动性、汇率或政策变化是否改变风险偏好？
7. ❓ 消息面有无重大利空？（减持、处罚、业绩变变脸等）
"""
...
"""
    if news_context:
        prompt += f"""
以下是 **{stock_name}({code})** 近{news_window_days}日的新闻搜索结果（包含个股、行业与宏观），请重点提取：
1. 🚨 **风险警报**：减持、处罚、利空
2. 🎯 **利好催化**：业绩、合同、政策
3. 📊 **业绩预期**：年报预告、业绩快报
4. 🌐 **宏观变化**：美联储/央行利率、通胀、美债/美元/汇率、政策与流动性变化，并说明对估值、资金面或风险偏好的影响
5. 🕒 **时间规则（强制）**：
